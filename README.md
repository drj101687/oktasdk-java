...
#### DNS TTL Caching
When using a [Security Manager](https://docs.oracle.com/javase/tutorial/essential/environment/security.html) or Java <1.6, the default JDK DNS cache policy is set to cache forever. This can cause service issues if the server at your cached IP goes down. We recommend setting a TTL of 30 seconds using any of these methods:

1. Append `-Dsun.net.inetaddr.ttl=30` when you start your jar.
2. Include `java.security.Security.setProperty("networkaddress.cache.ttl" , "30")` before any requests are made. Note: this won't work from a war, because the http stack starts before your war is loaded.
3. Uncomment and set `networkaddress.cache.ttl` to 30 in `$JRE_HOME/lib/security/java.security`.
